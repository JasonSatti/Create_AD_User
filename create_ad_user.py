...
def get_ad_user_info(dc):
    ...
    employee_manager_name = input(
        ...
        'First Last): ').lower()
    employee_title = input(
        'What is the new employee\'s job title? ').title()
    ...
    attrs = {
        ...
        "userprincipalname": employee_full_adname + '@hostname.dc.com',
        "mail": employee_email,
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Connect to AD Account')
    parser.add_argument('-un', '--username', required=False, type=str,
                        help='Username of AD domain admin in the format '
                             'fist.last. '
                             'Mandatory on first run, optional after if '
                             '-ru/--remember flag is used.')
    parser.add_argument('-pw', '--password', required=True, type=str,
                        help='Password of AD domain admin.')
    parser.add_argument('-ru', '--remember', action='store_true',
                        default=False,
    ...
    F'(sAMAccountName={employee_full_adname}))'):
        ...
        conn.extend.microsoft.modify_password(new_employee, 'default password')
